...
class ATest4:
    ...
    def __eq__(self, other) -> bool:
        ...
        _ATest4 = other
        result1 = True
        if not (self.anInt == "?DNC?" or _ATest4.anInt == "?DNC?"):
            if not _ATest4.anInt == self.anInt:
                result1 = False
        if not (self.aString == "?DNC?" or _ATest4.aString == "?DNC?"):
            if not _ATest4.aString == self.aString:
                result1 = False
        if not (self.aDouble == "?DNC?" or _ATest4.aDouble == "?DNC?"):
            if not _ATest4.aDouble == self.aDouble:
                result1 = False
        return result1
    ...
    @staticmethod
    def list_to_json(list: List["ATest4"]) -> str:
        json_builder = []
        json_builder.append("[")
    ...
    @staticmethod
    def list_from_json(json: str) -> List["ATest4"]:
        result_list = []
        json = re.sub(r"\s+", "", json) 
        json = json.replace("[", "").replace("]", "")
        json_objects = re.split(r"(?<=}),\s*(?={)", json)
        for json_object in json_objects:
            result_list.append(ATest4.from_json(json_object))
        return result_list
