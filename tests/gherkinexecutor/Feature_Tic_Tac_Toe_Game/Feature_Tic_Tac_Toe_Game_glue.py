from tests.gherkinexecutor.Feature_Tic_Tac_Toe_Game.MoveIn import MoveIn
from tests.gherkinexecutor.ID import ID
from tests.gherkinexecutor.TemperatureCalculations import TemperatureCalculations
from tests.gherkinexecutor.Feature_Tic_Tac_Toe_Game import *
from typing import List
import sys
from tests.gherkinexecutor.TicTacToeGame import TicTacToeGame

class Feature_Tic_Tac_Toe_Game_glue :
    DNCString = "?DNC?"

    def __init__(self):
        self.game = TicTacToeGame()
    def log(self, value):
        try:
            with open("log.txt", "a") as my_log:
                my_log.write(value + "\n")
        except IOError:
            print("*** Cannot write to log", file=sys.stderr)
    

    def Given_board_is(self, values: List[List[str]]) -> None:
        print("---  " + "Given_board_is")
        self.log("---  " + "Given_board_is")
        self.log(str(values))
        for value in values:
            print(value)
        self.game.set_board(values)

    def When_move_is(self, values: List[Move]) -> None:
        print("---  " + "When_move_is")
        self.log("---  " + "When_move_is")
        self.log(str(values))
        for value in values:
            print(value)
             # Add calls to production code and asserts
            i = value.to_MoveInternal()
            self.game.make_a_move(i.row, i.column, i.mark)

    def Then_board_is_now(self, value: str) -> None:
        print("---  " + "Then_board_is_now")
        self.log("---  " + "Then_board_is_now")
        self.log(str(value))
        print("Value is ")
        print(value)
        result = str(self.game)
        print("Result is")
        print(result)
        assert result == value

    def When_one_move_is(self, values: List[List[str]]) -> None:
        print("---  " + "When_one_move_is")
        self.log("---  " + "When_one_move_is")
        self.log(str(values))
        for value in values:
            try:
                parts = value[0].split(",")
                m = None
                if len(parts) == 3:
                    m = Move(parts[0], parts[1], parts[2])
                else:
                    raise ValueError("Invalid move string format")
                one_move = m.to_MoveInternal()
                self.game.make_a_move(one_move.row, one_move.column, one_move.mark)
            except Exception as e:
                print(f"Argument error: {v} {e}")


    def When_moves_are(self, values: List[Move]) -> None:
        print("---  " + "When_moves_are")
        self.log("---  " + "When_moves_are")
        self.log(str(values))
        for value in values:
            print(value)
            i = value.to_MoveInternal()
            self.game.make_a_move(i.row, i.column, i.mark)

